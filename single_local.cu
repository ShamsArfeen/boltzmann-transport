...
__host__ static inline double number_density(void) {
    if (MASS == 0.0)
        return pow(TEMPERATURE, 3) / (PI * PI);

    double sum = 0.0;
    double dt = 1e-3;
    double beta_m = MASS / TEMPERATURE;

    for (int i = 0; i < 100000; i++) {
        double t = 1.0 + i * dt;
        sum += exp(-beta_m * t) * pow(t*t - 1.0, 1.5);
    }
    return sum * dt * pow(MASS, 4) / (2.0 * PI * PI * TEMPERATURE);
}
    ...
    int rng_shift)
{
    ...
    double prob = FM2 * SIGMA_FM2 * DT * vrel / sys->DV;
